...
#if defined(NDEBUG)
# error "Snatcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6a3035338d3597ac3d7aeaad5bc9c92d04e538e68014cf5441077e247ee952a8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Snatcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Snatcoin Signed Message:\n";
...
{
    // Snatcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // Snatcoin: 60 minutes
static const int64 nTargetSpacing = 1 * 60; // Snatcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Snatcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Snatcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5e23b3362ddefde8491faa9a5202882b4bcf6e8fe4ffc9a806ac051bfb32f237");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Snatcoin - A New Beginning";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a6f2a7bbce0a6e41721e16752502b076a39d51c7ed1ba6b87424564dc235d42f2a2b54d8e8b4fe579c7911dfed23ed760944b58e0ec4ad408b4cd006bf1f66c0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517863503;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 202722;

        if (fTestNet)
        {
            block.nTime    = 1517863488;
            block.nNonce   = 787487;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3d99c5aff194c94de717d2ba7c89c578f5152f10808d4c31e730ed481c6c6f3c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc8, 0xb8, 0xde }; // Snatcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SnatcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SnatcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SnatcoinMiner(CWallet *pwallet)
{
    printf("SnatcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("snatcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SnatcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SnatcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SnatcoinMiner, pwallet));
}
